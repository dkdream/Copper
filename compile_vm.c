...
static bool Node_compile_vm(FILE* ofile, char* name, unsigned index, Node *node, unsigned* current)
{
    ...
    case Action:
        {
            ...
            fprintf(ofile,
                    "static struct prs_label     ll_%s_%u_arg = { &event_%s, \"event_%s\" };\n"
                    "static struct prs_node      ll_%s_%u     = { 0,0, prs_Event, (union prs_arg) (&ll_%s_%u_arg) };\n",
                    name, index, event, event,
                    name, index, name, index
                    );
            *current = index;
        }
    ...
    case Mark:
        {
            const char* marker = node->mark.name;
            if (!strcmp("end", marker)) {
                fprintf(ofile,
                        "static struct prs_node   ll_%s_%u     = { 0,0, prs_End, };\n",
                        name, index
                        );
                *current = index;
            } else if (!strcmp("begin", marker)) {
                fprintf(ofile,
                        "static struct prs_node   ll_%s_%u     = { 0,0, prs_Begin };\n",
                        name, index
                        );
                *current = index;
            } else {
