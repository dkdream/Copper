                      ...
                      PrsInput input)
{
    ...
    inline bool checkFirstSet() {
        ...
        if (!first) {
            indent(4); CU_DEBUG(4, "checkFirst(%s) at (%u,%u) (skipped no first)\n",
                                node_label(),
                                at.line_number + 1,
                                at.char_offset);
        ...
        if (list) {
            if (0 < list->count) {
                indent(4); CU_DEBUG(4, "checkFirst(%s) at (%u,%u) (skipped call nodes)\n",
                                    node_label(),
                                    at.line_number + 1,
                                    at.char_offset);
                return true;
        ...
        PrsChar chr = 0;
        if (!current(&chr)) {
            indent(4); CU_DEBUG(4, "checkFirst(%s) at (%u,%u) (skipped eof)\n",
                                node_label(),
                                at.line_number + 1,
                                at.char_offset);
            return true;
