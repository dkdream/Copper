...
static void Rule_compile_c2(Node *node, int export_all)
{
  assert(node);
  ...
  if (node->rule.next)
      Rule_compile_c2(node->rule.next, export_all);
}
...
void Rule_compile_c_preamble(FILE* ofile)
{
    if (!ofile) return;

    fprintf(ofile, "/* A recursive-descent parser generated by copper %d.%d.%d */\n", COPPER_MAJOR, COPPER_MINOR, COPPER_LEVEL);
    fprintf(ofile, "\n");
    fprintf(ofile, "%s", preamble);
    fprintf(ofile, "\n");
}
...
void Rule_compile_c_declare(FILE* ofile, Node *node) {
    ...
    for (current = node; current;  current = current->rule.next)
        if (RuleExported & current->rule.flags)
            fprintf(ofile, "extern int yy_%s (YYClass* yySelf, YYStack* yystack);\n",
                    current->rule.name);

    fprintf(ofile, "\n");
}

void Rule_compile_c(Node *node, int no_preamble, int export_all)
{
  Node *current;
  ...
  */
  ...
  if (no_preamble) {
      ...
      Rule_compile_c_preamble(output);
  }
  ...
  Rule_compile_c2(node, export_all);
}
